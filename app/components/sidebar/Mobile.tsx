...
const Mobile: FC<MobileTypes> = ({ navMobile, setNavMobile, password }) => {
  return (
    <div
      className={`${
        navMobile &&
        "lg:hidden fixed left-0 top-0 w-full h-screen bg-primary/80 z-80 ease-in-out duration-700 transition"
      }`}
    ...
    >
      <div
        className={
          navMobile
            ? "fixed top-0 left-0 h-screen bg-main bg-primary shadow-xl shadow-white w-[80%] md:w-[70%] ease-in duration-500 z-80 pt-3 px-3"
            : "fixed -left-[100%] top-0 ease-in duration-300 bg-primary/95 shadow-xl shadow-white w-[80%] md:w-[50%] h-screen z-80 pt-3 px-3"
        }
